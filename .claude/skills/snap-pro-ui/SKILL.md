...
## 10. Approval & commit workflow

The repo follows an **approval-gated commit** model. Agents do not commit on
their own — they wait for an explicit go-ahead, then ship.

**Trigger phrase**: when the user replies with `approved` (case-insensitive,
either as the entire message or as the first word), the agent MUST:

1. Run `git status --short` to confirm there are pending changes.
2. Stage everything: `git add -A`.
3. Write a **short, descriptive Conventional Commit** message that summarises
   the work just completed in this turn. Format:
   ...
   - `type` ∈ `feat | fix | style | refactor | perf | docs | chore`
   - `scope` reflects the area touched (`learn`, `nav`, `ui`, `hero`, `pricing`,
     `skill`, etc.)
   - Subject line ≤ 72 chars, imperative mood, no trailing period
   - Body bullets only when there's more than one notable change

4. Push to the current branch: `git push origin <branch>`.
5. Reply with one line: the commit SHA and a one-sentence recap.

**Do not commit without `approved`.** Other phrases ("looks good", "ship it",
"go ahead") are not triggers — only the literal word `approved` counts. This
keeps the user firmly in control of what lands on the remote.

**Never** include emoji or "Generated by …" / "Co-Authored-By: Claude" /
"Co-Authored-By: Copilot" footers in commit messages. Commits should read like
they came from the human reviewer.

If lint or build is broken, stop and report — do not push a red commit even
after `approved`.
